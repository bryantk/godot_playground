...
func _ready() -> void:
	...
	for cell in [Vector3i(14, 0, 12), Vector3i(14, 0, 13), Vector3i(0, 0, 9), Vector3i(14, 0, 9)]:
		...
		var raw: Variant = td.get_custom_data(Passability.DATA_PATHING) if td != null else "<no tile>"
		print("   %s  tile=%s pathing=%s dirs=%s can_enter=%s" % [
			cell, td != null, raw, Passability.directions(ctx, cell), Passability.can_enter(ctx, cell, player)])
